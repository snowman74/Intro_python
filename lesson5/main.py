# from divisor_master import canon_factorization as c_f, top_simple_devisor as ts_d, simple_number as s_n, devisors_list, top_devisor
# print(c_f(999))
# print(ts_d(973))
# print(s_n(499))
# print(devisors_list(900))
# print(top_devisor(798))
import divisor_master
print(divisor_master.simple_number(59))
print(divisor_master.canon_factorization(999))
print(divisor_master.top_simple_devisor(973))
print(divisor_master.simple_number(499))
print(divisor_master.devisors_list(900))
print(divisor_master.top_devisor(798))